        ...
        with codecs.open(outdir, 'w', self.code) as f_out:
            for obj in objects:
                ...
                if (half_iou == 1):
                    ...
                    # if min(polyInsub) <= 1 or max(polyInsub) >= min(right - left, down - up):
                    #     box1 = to_rrect(np.float32(polyInsub.reshape(4, 2)))
                    #     poly1 = shgeo.Polygon(box1.reshape(4, 2))
                    #     poly = shgeo.Polygon(polyInsub.reshape(4, 2))
                    #     poly1_1 = poly1.intersection(poly)
                    #     if poly1_1.area / poly1.area < 0.85:
                    #         polyInsub = box1.reshape(8).astype(int)  
