...
class BinaryMaskList(object):
    ...
    def __init__(self, masks, size):
        ...
        self.keypoints = masks
        self.size = tuple(size)

    def transpose(self, method):
        dim = 1 if method == FLIP_TOP_BOTTOM else 2
        flipped_masks = self.keypoints.flip(dim)
    ...
    def crop(self, box):
        ...
        cropped_masks = self.keypoints[:, ymin:ymax, xmin:xmax]
    ...
    def resize(self, size):
        ...
        resized_masks = interpolate(
            input=self.keypoints[None].float(),
            size=(height, width),
            mode="bilinear",
            align_corners=False,
        )[0].type_as(self.keypoints)
    ...
    def _findContours(self):
        contours = []
        masks = self.keypoints.detach().numpy()
        for mask in masks:
            mask = cv2.UMat(mask)
    ...
    def __getitem__(self, index):
        ...
        return BinaryMaskList(self.keypoints[index], self.size)
...
class SegmentationMask(object):
    ...
    def get_mask_tensor(self):
        ...
        return instances.keypoints.squeeze(0)
