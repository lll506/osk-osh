...
class RandomSquareRotate( object ):
    ...
    def __call__( self, image, target=None ):
        ...
        polygons = list( map( lambda x: x.polygons[0], keypoints.instances.polygons ) )
        ...
        new_keypoints = SegmentationMask( new_points.reshape((-1, 1, 8)).tolist(), image.size, mode='poly' )
...
class ToRect( object ):
    ...
    def __call__( self, image, target=None ):
        ...
        keypoints = target.get_field( "keypoints" )
        polygons = list( map( lambda x: x.polygons[0].numpy(), keypoints.instances.polygons ) )
        ...
        new_target = BoxList( boxes, image.size, mode="xyxy" )
        new_target._copy_extra_fields( target )
        new_keypoints = SegmentationMask( rrects_np.reshape( (-1, 1, 8)).tolist(), image.size, mode='poly' )
        new_target.add_field( "keypoints", new_keypoints )
...
class SortForQuad( object ):
    ...
    def __call__( self, image, target=None ):
        ...
        new_keypoints = SegmentationMask( new_polygons, image.size, mode='poly' )
        target.add_field( "keypoints", new_keypoints )
